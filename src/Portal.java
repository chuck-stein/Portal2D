...
public class Portal extends AbstractPortalObj {
  ...
  /**
   * Returns true if the space indicated by the given x, y, width, and height is fully within this
   * Portal lengthwise, as well as touching this Portal
   *
  ...
  public void transport(PhysicsObj obj, Portal exit, float vx, float vy) {
    ...
    exit.receive(obj, inV, crossV, orientation);
   ...
   * @param crossV        the velocity of the PhysicsObj in the direction perpendicular to which the
   *                      Portal that transported the object was facing, at the time that it entered
   *                      that Portal
   * @param inOrientation the orientation of the Portal that transported to this one
   */
  public void receive(PhysicsObj obj, float inV, float crossV, PortalOrientation inOrientation) {
    if (inOrientation == PortalOrientation.FROM_TOP && orientation == PortalOrientation.FROM_TOP
            && inV > 14) {
      inV = 14; // so that you can not use two floor portals to infinitely jump higher and higher
    }
